  ...
  Future<Map<String, dynamic>> obtenerResumenMetricas(int usuarioId) async {
    try {
      ...
      for (var productoId in todosProductosIds) {
        final metricasProductoActual = metricasPorProductoActual[productoId] ?? [];
        
        final metricasProductoAnterior = metricasPorProductoAnterior[productoId] ?? [];
        ...
        double prodVariacionIngresos = productIngresosAnterior > 0
            ? ((productIngresos - productIngresosAnterior) / productIngresosAnterior) * 100 
            : 0;
            
        double prodVariacionGastos = productGastosAnterior > 0 
            ? ((productGastos - productGastosAnterior) / productGastosAnterior) * 100 
            : 0;
            
        double prodVariacionBeneficio = productBeneficioAnterior > 0 
            ? ((productBeneficio - productBeneficioAnterior) / productBeneficioAnterior) * 100 
            : 0;
            
        double prodVariacionVentas = productVendidosAnterior > 0 
            ? ((productVendidos - productVendidosAnterior) / productVendidosAnterior) * 100 
            : 0;
        
        // Añadir resumen de este producto
